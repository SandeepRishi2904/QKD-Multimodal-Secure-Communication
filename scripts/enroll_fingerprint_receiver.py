#!/usr/bin/env python3
"""
Fingerprint Enrollment Script - Receiver
One-time setup to enroll receiver's fingerprint template
"""
import sys
import logging
...
def main():
    print("=" * 60)
    print("🔐 QKD MULTIMODAL SYSTEM - Fingerprint Enrollment (Receiver)")
    print("=" * 60)
    print()
    ...
    success, message = auth.enroll_fingerprint(identity='receiver')

    print()
    if success:
        print("✅ " + message)
        print("\nYour fingerprint template has been saved securely.")
        print("You can now use fingerprint authentication as receiver.")
    else:
